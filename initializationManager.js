...
export class InitializationManager {
    ...
    initializeTasks() {
            ...
            action: async () => {
                ...
                try {
                    ...
                    if (questProgressManager?.initializeQuestProgress) {
                        try {
                            // Pass null — the real goal is generated by generateAdventureGoal
                            // after the initial story runs. Passing 'Not set yet.' here would
                            // register the placeholder as an active objective.
        ...
        // Generate a concrete adventure goal AFTER the game screen is already
        // showing (FINALIZE phase, low priority). This keeps the screen-appear
        // timing fast — players see the story immediately, then the goal text
        // fills in from "Setting your quest..." to the real goal within seconds.
        this.registerTask('generateAdventureGoal', {
            phase: this.phases.FINALIZE,
            dependencies: ['gameState', 'generateInitialStory', 'finalizeUI'],
            priority: 10,
            timeout: 30000,
            ...
            action: async () => {
                ...
                if (gameState.adventureGoal && gameState.adventureGoal !== 'Not set yet.') {
                    this.log(`InitManager: adventureGoal already set to "${gameState.adventureGoal}" — skipping goal generation.`);
                    if (gameState.questProgressManager) {
                        gameState.questProgressManager.updateObjectives([gameState.adventureGoal], true);
                    }
                    UI.updateGameHeader();
                    return { goalSet: true, source: 'existing' };
                }
...
Respond with ONLY a JSON object: {"goal": "Your specific goal sentence here."}`;
                ...
                try {
                    const result = await API.getAIResponseJSON(
                        [
                            { role: 'system', content: 'You are a game master. Respond only with the requested JSON.' },
                    ...
                    if (goal && goal.length > 5) {
                        ...
                        UI.updateGameHeader();
                        this.log(`InitManager: Adventure goal set — "${goal}"`);
                        return { goalSet: true, goal, source: 'ai' };
                    }
                    throw new Error('AI returned empty or invalid goal');
                } catch (e) {
                    ...
                    const fallbacks = {
                        ...
                        jungle: 'Reach the hidden temple at the heart of the jungle.',
                        future_utopia: 'Expose the dark secret beneath the perfect society.',
                        dinosaur: 'Escape the island and warn the world of what lurks here.',
                        arctic: 'Survive the frozen wilderness and find a way home.',
                        steampunk: 'Stop the rogue automaton before it destroys the city.',
                        haunted: 'Break the curse and free the souls trapped in this place.',
                        cyberpunk: 'Expose the corporation\'s crimes before they silence you.',
                        wild_west: 'Bring justice to the lawless frontier.',
                        post_apoc: 'Find the sanctuary rumored to hold the key to rebuilding civilization.',
                    ...
                    UI.updateGameHeader();
                    this.log(`InitManager: Goal AI failed (${e.message}), using fallback: "${fallbackGoal}"`);
                    return { goalSet: true, goal: fallbackGoal, source: 'fallback' };
                }
            },
            description: 'Generate the main adventure goal'
        });
