...
export class EncounterSystem {
    ...
    static processCombatEncounter(encounter) {
        ...
        try {
            // BUG-20 fix: previously `gameState.enemies = encounter.enemies`
            // wiped any narrator-spawned enemies (including bosses) when the
            // legacy random-encounter system fired the same turn. If the
            // narrator already populated enemies via /enemies/-, skip the
            // legacy spawn entirely — the narrator's intent wins.
            const liveExisting = (gameState.enemies || []).filter(e => e && !e.isDefeated);
            if (liveExisting.length > 0) {
                log(`processCombatEncounter: narrator already spawned ${liveExisting.length} enemies; skipping legacy encounter overlay.`);
                return true;
            }
    ...
    static processBossEncounter(encounter) {
        ...
        try {
            // BUG-20 fix: same protection as processCombatEncounter — don't
            // overwrite narrator-spawned bosses with a legacy roll.
            const liveExisting = (gameState.enemies || []).filter(e => e && !e.isDefeated);
            if (liveExisting.length > 0) {
                log(`processBossEncounter: narrator already spawned ${liveExisting.length} enemies; skipping legacy boss overlay.`);
                return true;
            }
            gameState.enemies = encounter.enemies;
    ...
    /**
     * Processes an anomaly encounter
