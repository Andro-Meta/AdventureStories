...
const PATHS = [
    ...
    {
        regex: /^\/questProgress\/sideQuests\/([a-zA-Z0-9_-]+)\/(completed|progress)$/,
        ops: ['replace'],
        validate: (m, value, gs) => {
            const id = m[1];
            const field = m[2];
            const list = gs.questProgress?.sideQuests || [];
            if (!list.some(q => q?.id === id)) return `sideQuest ${id} not found`;
        ...
        apply: (m, value, gs) => {
            const id = m[1];
            const field = m[2];
            const quest = gs.questProgress.sideQuests.find(q => q?.id === id);
            if (!quest) return `sideQuest ${id} vanished`;
            quest[field] = value;
            if (field === 'completed' && value === true) {
                quest.turnCompleted = gs.turn;
                quest.progress = 100;
                return `sideQuest "${quest.name}" completed`;
            }
 ...
 * sequentially. If any op fails validation, the whole batch is rejected
 * BEFORE any mutation (atomic-ish: all-or-nothing within a turn). Returns a
 * summary string per applied op.
 */
